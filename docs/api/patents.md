...
5. **Apply T₀ filter + per-row feature engineering** downstream in `features.transforms.build_features` — produces a leakage-free intermediate frame (per-row aggregates, raw categoricals, NaN-bearing nullable scalars). Cross-row encodings (one-hot, frequency, median imputation) live in `models.preprocessing.build_preprocessor` and are refit per CV fold. T₀-filter on `event_dates`/`assignment_received_dates` runs here, once.
