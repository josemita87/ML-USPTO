...
5. **Apply T₀ filter + per-row feature engineering** downstream in `features.transforms.build_features` — produces a leakage-free intermediate frame (per-row aggregates, raw categoricals, NaN-bearing nullable scalars). Cross-row encodings split: rolling priors and frequency counts are computed once over the full corpus by `models.preprocessing.attach_rolling_encodings` (leakage-safe via strict-`<` T₀ gate); only OHE + median imputation in `build_preprocessor` are refit per CV fold. T₀-filter on `event_dates`/`assignment_received_dates` runs here, once.
